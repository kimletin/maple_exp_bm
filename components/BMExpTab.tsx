        ...
        <table className="w-full text-sm border-collapse">
          <thead className="sticky top-0">
            <tr className="bg-gray-50 dark:bg-zinc-800 border-b border-gray-200 dark:border-zinc-600">
              <th className="text-center px-5 py-2 text-gray-600 dark:text-zinc-400 font-medium">{levelLabel}</th>
              <th className="text-center px-5 py-2 text-gray-600 dark:text-zinc-400 font-medium">경험치</th>
              {showPct && <th className="text-center px-5 py-2 text-gray-600 dark:text-zinc-400 font-medium">경험치 배율</th>}
            </tr>
          </thead>
          <tbody>
            {rows.map(row => (
              ...
              >
                <td className={'px-5 py-1.5 text-center ' + (row.isMe ? row.textColor : 'text-gray-700 dark:text-zinc-300')}>
                  {row.level}
                ...
                {showPct && (
                  <td className={'px-5 py-1.5 text-center text-xs ' + (row.isMe ? row.textColor : 'text-gray-400 dark:text-zinc-500')}>
