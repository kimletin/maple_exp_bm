...
export default function Num({ n, className }: NumProps) {
  ...
  return (
    <span className={`relative group inline-block ${className ?? ''}`}>
      ...
      <span className="pointer-events-none absolute bottom-full left-1/2 -translate-x-1/2 mb-1.5 px-2 py-1 rounded bg-gray-800 dark:bg-zinc-700 text-white text-xs whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity z-50 shadow-lg">
        {exact}
      </span>
    </span>
  );
