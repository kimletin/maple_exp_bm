    ...
    <div className="bg-white dark:bg-zinc-900 rounded-xl border border-gray-100 dark:border-zinc-700 shadow-sm overflow-hidden">
      ...
      <div className="p-6 space-y-5">
        ...
        <Section title="2. 처리하는 개인정보의 항목">
          ...
          <p>① 캐릭터 닉네임: 넥슨 OpenAPI 조회 목적으로만 사용되며, 서비스 서버에 영구 저장되지 않습니다. 다만 조회 결과(공개된 게임 정보)는 API 호출 부하를 줄이기 위해 서버 및 CDN에 일정 시간 동안 임시 캐시될 수 있으며, 이는 개인 식별 목적으로 사용되지 않고 캐시 기간이 지나면 자동으로 갱신·만료됩니다.<br />② 브라우저 저장 정보: 캐릭터 설정값, 환경설정 등은 이용자 브라우저의 로컬 저장소(localStorage)에만 저장되어 이용자 기기에 보관됩니다.</p>
          <p>서비스는 처리 목적에 필요한 최소한의 정보만 이용합니다.</p>
        </Section>

        <Section title="3. 개인정보의 처리 및 보유 기간">
          <p>서비스는 이용자의 개인정보를 서버에 영구 보관하지 않습니다. 캐릭터 조회 결과(공개된 게임 정보)는 성능을 위해 서버 및 CDN에 단기간 캐시될 수 있으나, 캐시 기간이 지나면 자동으로 갱신·만료됩니다. 브라우저에 저장된 정보는 이용자가 삭제하기 전까지 이용자 기기에만 보관되며, 이용자가 브라우저 데이터를 삭제하면 즉시 삭제됩니다.</p>
        </Section>
        ...
        <Section title="6. 개인정보의 안전성 확보조치">
          ...
          <p>① 이용자의 개인정보를 서버에 영구 저장하지 않아 유출 위험을 원천적으로 최소화합니다. 조회 결과의 임시 캐시 또한 개인 식별 정보가 아닌 공개된 게임 정보에 한하며 일정 시간 후 자동 만료됩니다.<br />② 넥슨 OpenAPI 통신 및 사이트 접속은 암호화(HTTPS) 통신을 통해 이루어집니다.<br />③ 처리 목적에 필요한 최소한의 정보만 처리하는 내부 원칙을 운영합니다.</p>
        </Section>
